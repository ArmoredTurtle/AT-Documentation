...
AFC will run a self-test automatically on startup. It will run through the respoolers on each lane and will update the
lightbox LED indicating status for each lane.

=== "BoxTurtle"

    The self-test runs through lanes 1-4, from left to right.
